...
if (sloc.indexOf("://uat") != -1)
		environment = "uat";
	else if (sloc.indexOf("://dev") != -1)
		environment = "dev";
	else  if ((sloc.indexOf("://local") != -1) || (sloc.indexOf("://127.0.0.1") != -1))
		environment = "local";
*/

var s_account="westernunionglobal";
	 	   	  
if (environment && environment=="prod") {
	s_account="westernunioncorporate";
}
...
s.linkInternalFilters="javascript:,wuintranet"
...
function s_doPlugins(s) {
	if (s.getQueryParam('pid')) s.eVar23 = s.getQueryParam('pid')
	if (s.getQueryParam('gnd')) s.eVar23 = s.getQueryParam('gnd')
	if (!s.campaign)   s.campaign = s.getQueryParam('src')

s.linkLeaveQueryString=true;
	var url=s.linkHandler('pid');
	s.pidValue=s.getQueryParam('pid','',url);
	if(url){
 	   s.eVar23=s.pidValue.replace(')','').replace("'","")
 	   s.linkTrackVars="eVar23"
	}

	if (s.eVar23 != null && s.eVar23.indexOf("#") != -1)
	{
		s.eVar23=s.eVar23.substring(0,s.eVar23.indexOf("#"));
	}
	...
	s.prop6=s.getTimeParting('h','0',strYear);
	s.prop7=s.getTimeParting('d','0',strYear);
	s.prop8=s.getTimeParting('w','0',strYear);
	s.eVar24=s.getTimeParting('h','0',strYear);
	s.eVar25=s.getTimeParting('d','0',strYear);
	s.eVar26=s.getTimeParting('w','0',strYear);
...
s.visitorNamespace="westernunion"
s.trackingServer="westernunion.112.2o7.net"
